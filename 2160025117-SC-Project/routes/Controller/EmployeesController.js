...
exports.getAllJobsByEmployee = async (req, res) => {
    // check if the user is logged in or not
    const user = await User.findOne({ user_id: req.body.user_id });
    if (user) {
...
exports.getJobAppliedByEmployee = async (req, res) => {
    // check if the user is logged in or not
    const user = await User.findOne({ user_id: req.body.user_id });
    if (user) {
        //const applications = await EmployeeJobs.find({ employee_id: user._id });
...
exports.changeProfileToEmployer = async (req, res) => {
...
exports.deleteAllEmployeeJobApplicatoin = async (req, res) => {
    // check if the user is logged in or not
    const user = await User.findOne({ user_id: req.body.user_id });
    if (!user) return res.status(200).send("Access denied, please login to access this page");
